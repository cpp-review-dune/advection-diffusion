...
template <int dim> Tensor<1, dim> beta(const Point<dim> &p) {
...
template <int dim> void AdvectionProblem<dim>::assemble_system() {
...
template <int dim> void AdvectionProblem<dim>::refine_grid() {
  ...
  DerivativeApproximation::approximate_gradient(mapping, dof_handler, solution,
                                                gradient_indicator);
...
void AdvectionProblem<dim>::output_results(const unsigned int cycle) const {
...

  {
    ...
    VectorTools::integrate_difference(mapping, dof_handler, solution,
                                      Functions::ZeroFunction<dim>(), values,
                                      quadrature, VectorTools::Linfty_norm);
    const double l_infty = VectorTools::compute_global_error(
        triangulation, values, VectorTools::Linfty_norm);
